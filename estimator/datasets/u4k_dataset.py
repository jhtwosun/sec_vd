...
class UnrealStereo4kDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            crop_images = self.resize(dict(image=image_tensor))['image']
            ...
            return_dict = \
                {#'image_lr': image_lr_tensor, 
                 'image_hr': crop_images, # save some memory
                 #'crops_image_hr': crop_images, 
                 'depth_gt': depth_gt_crop_tensor, 
                 'disp_gt': disp_gt_crop_tensor,
                 #'crop_depths': crop_depths,
                 #'bboxs': bboxs,
                 'img_file_basename': img_file_basename}
        else:
            image_hr_tensor = self.resize(dict(image=image_hr_tensor))['image']
