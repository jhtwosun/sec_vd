...
from .transformers import Resize as ResizeDA
from .transformers import NormalizeImage
...
class MVSSynthDataset(Dataset):
    ...
    def max_disparity_adjust(self, d, max_disparity_mu = 1.2, max_disparity_sigma = 0.2):
        ...
        if split is not None:
            with open(split) as f:
                for line in f:
                    ...
                    img, depth_map, ext_name = line.strip().split(" ")

                    # HACK: a hack to replace the png with raw to accelerate training
                    img = img[:-3] + 'raw'
                    depth_map = depth_map[:-3] + 'npy'

                    img_info['depth_map_path'] = osp.join(data_root, depth_map)
                    img_info['img_path'] = osp.join(data_root, img)
                    img_info['filename'] = img

                    img_info['depth_fields'] = []
                    with open(osp.join(data_root, ext_name), "r") as ext_json:
                        ext = json.load(ext_json)

                    img_info['depth_factor'] = ext['f_x']
    ...
    def __getitem__(self, idx):
        ...
        image = np.fromfile(open(img_file_path, 'rb'), dtype=np.uint8).reshape(1080, 1920, 3) 
        
        # load depth
        # disp_gt = np.load(disp_path, mmap_mode='c').astype(np.float32)
        # depth_gt = depth_factor / disp_gt
        depth_gt = np.load(depth_path, mmap_mode='c').astype(np.float32)
        valid_mask_depth = depth_gt != 0.0
        disp_gt = np.where(valid_mask_depth, 1.0 / depth_gt, 0.0) * depth_factor
        # max_disparity = max_disparity_adjust(np.max(ref_depth_full))
        # disp_gt = disp_gt / max_disparity
        
        image = self.resize(dict(image=image))['image']
        if self.mode == 'train':
        ...
        if self.mode == 'train':
            ...
            return_dict = \
                {#'image_lr': image_lr_tensor, 
                 'image_hr': image_hr_tensor, # save some memory
    ...
    def get_metrics(self, depth_gt, result, disp_gt_edges, **kwargs):
        depth_gt = (depth_gt - depth_gt.min(dim=-1, keepdim=True)[0]) / (depth_gt.max(dim=-1, keepdim=True)[0] - depth_gt.min(dim=-1, keepdim=True)[0])
        
        return compute_metrics(depth_gt, result, disp_gt_edges=None, min_depth_eval=self.min_depth, max_depth_eval=self.max_depth, garg_crop=False, eigen_crop=False, dataset='')
