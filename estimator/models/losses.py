...
class AffineInvariantLoss(nn.Module):
    """Affine Invariant Loss (pixel-wise)"""
    def __init__(self, **kwargs):
        super(AffineInvariantLoss, self).__init__()
        self.name = 'AffineInvariant'

    def forward(self, input, target, min_depth=None, max_depth=None, additional_mask=None):
        
        # assert input (prediction) is dispariy, and target is depth map
        b, _, h_i, w_i = input.shape
        b, _, h_t, w_t = target.shape
        ...
        input_disparity = input.view(b, -1)
        target_disparity = target.view(b, -1)

        # Normalize to 0-1
        input_disparity = (input_disparity - input_disparity.min(dim=-1, keepdim=True)[0]) / (input_disparity.max(dim=-1, keepdim=True)[0] - input_disparity.min(dim=-1, keepdim=True)[0])
        target_disparity = (target_disparity - target_disparity.min(dim=-1, keepdim=True)[0]) / (target_disparity.max(dim=-1, keepdim=True)[0] - target_disparity.min(dim=-1, keepdim=True)[0])

        # Calculate t(d) and s(d)
        t_input = torch.median(input_disparity, dim=-1, keepdim=True)[0]
        t_target = torch.median(target_disparity, dim=-1, keepdim=True)[0]
        
        s_input = torch.mean(torch.abs(input_disparity - t_input), dim=-1, keepdim=True)
        s_target = torch.mean(torch.abs(target_disparity - t_target), dim=-1, keepdim=True)

        # Scale and shift predictions and ground truth
        input_scaled = (input_disparity - t_input) / s_input
        target_scaled = (target_disparity - t_target) / s_target

        # Compute affine-invariant mean absolute error loss
        loss = torch.mean(torch.abs(input_scaled - target_scaled))
        ...
        if torch.isnan(loss):
            ...
            print_log("Input disparity min: {} max: {}".format(torch.min(input_disparity), torch.max(input_disparity)), logger='current')
            print_log("Input disparity scale min: {} scale max: {}".format(torch.min(s_input), torch.max(s_input)), logger='current')
            
            print_log("Target disparity min: {} max: {}".format(torch.min(target_disparity), torch.max(target_disparity)), logger='current')
            print_log("Target disparity scale min: {} scale max: {}".format(torch.min(s_target), torch.max(s_target)), logger='current')
