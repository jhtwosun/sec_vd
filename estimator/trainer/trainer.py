...
class Trainer:
    ...
    def __init__(
        ...
        self.amp_dtype = amp
        self.use_amp = True if self.amp_dtype in ['bf16', 'fp16'] else False
        
        if self.use_amp:
            dtype = dtype_dict.get(self.amp_dtype, 'float32')
            config.optim_wrapper['type'] = 'AmpOptimWrapper'
            config.optim_wrapper['dtype'] = dtype
            
        # build opt and schedule
        total_params  = sum(p.numel() for p in model.parameters() if p.requires_grad)
        
        print_log('Total trainable params: {}'.format(total_params), logger='current')
        ...
        print_log(f'collect_input_args: {self.collect_input_args}', logger='current')
    ...
    def log_images(self, log_dict, prefix="", scalar_cmap="turbo_r", min_depth=1e-3, max_depth=80, step=0):
        ...
        if 'depth_pred' in log_dict.keys():
            ...
            invalid_mask = np.zeros_like(invalid_mask)
